function collect_results(;method_name = "", dataset = "", path = "", preprocessing = "", dropout = 0.0, input_do = 0.0, wall_seed = 1, wall_epochs = 200, lr = "1e-3")
	...
	for seed = 1:wall_seed
		...
		test_errors[seed] = mean(resultfile[wall_epochs+1-20:wall_epochs+1, 3])
	end
	...
	print(f, string(input_do))
	for i = length(input_do)+1:8 print(f, " ") end
	print(f, wall_seed)
	...
	info("Method:", method_name,"   DATASET:", dataset, "   preprocessing:", preprocessing, "  dropout:", dropout, "  input dropout:", input_do, "   wall seed:", wall_seed, "    mean:", mean(test_errors), "  std:", std(test_errors)/sqrt(wall_seed))
	close(f)
end
